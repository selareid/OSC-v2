module.exports = function () {
    ...
    Room.prototype.buildThings =
        function (room = this) {
            ...
            if (storage) {
                if (!storage.pos.lookFor(LOOK_STRUCTURES, {filter: (s) => s.structureType == STRUCTURE_RAMPART})[0]) Memory.cs[room.name + ',' + storage.pos.x + ',' + storage.pos.y] = STRUCTURE_RAMPART;
                if (terminal) {
                    if (!terminal.pos.lookFor(LOOK_STRUCTURES, {filter: (s) => s.structureType == STRUCTURE_RAMPART})[0]) Memory.cs[room.name + ',' + terminal.pos.x + ',' + terminal.pos.y] = STRUCTURE_RAMPART;
                    _.forEach(global[this.name].spawns, (spawn) => {
                        if (!spawn.pos.lookFor(LOOK_STRUCTURES, {filter: (s) => s.structureType == STRUCTURE_RAMPART})[0]) Memory.cs[room.name + ',' + spawn.pos.x + ',' + spawn.pos.y] = STRUCTURE_RAMPART;
                    });
                    _.forEach(global[this.name].towers, (tower) => {
                        if (!tower.pos.lookFor(LOOK_STRUCTURES, {filter: (s) => s.structureType == STRUCTURE_RAMPART})[0]) Memory.cs[room.name + ',' + tower.pos.x + ',' + tower.pos.y] = STRUCTURE_RAMPART;
                    });
                }
                ...
                _.forEach(pathController, (pathData) => {
                    if (!new RoomPosition(pathData.x, pathData.y, room.name).lookFor(LOOK_STRUCTURES)[0]) {
                        var res = Memory.cs[room.name + ',' + pathData.x + ',' + pathData.y] = STRUCTURE_ROAD;
            ...
            function pathingAPlacement(s) {
                ...
                _.forEach(ret.path, (pos) => {
                    ...
                    if (!_.filter(new RoomPosition(pos.x, pos.y, room.name).lookFor(LOOK_STRUCTURES), (s) => s.structureType !== STRUCTURE_RAMPART)[0]) {
                        var res = Memory.cs[room.name + ',' + pos.x + ',' + pos.y] = STRUCTURE_ROAD;
